...
def export_standalone_figure_script(
    ...
    for axis_index, ax in enumerate(fig.axes):
        ...
        for artist in ax.get_children():
            ...
            if isinstance(artist, Line2D):
                ...
                image_ref = store_array(
                    artist.get_array(), f"ax{axis_index}_image_values"
                )
                kwargs = {
                    "cmap": artist.get_cmap().name,
                    "vmin": (
                        None if artist.norm.vmin is None else float(artist.norm.vmin)
                    ),
                    "vmax": (
                        None if artist.norm.vmax is None else float(artist.norm.vmax)
                    ),
                    "interpolation": artist.get_interpolation(),
                    "origin": artist.origin,
                    "extent": tuple(float(value) for value in artist.get_extent()),
                    "aspect": _python_value(ax.get_aspect()),
                    "zorder": float(artist.get_zorder()),
                }
                if artist.get_alpha() is not None:
                    kwargs["alpha"] = float(artist.get_alpha())
                lines.append(f"{axis_name}.imshow({image_ref}, **{kwargs!r})")
                ...
                for path_index, path in enumerate(paths):
                    ...
                    color = (
                        tuple(
                            float(v) for v in edgecolors[path_index % len(edgecolors)]
                        )
                        if len(edgecolors)
                        else "black"
                    )
                ...
                sizes_ref = store_array(
                    artist.get_sizes(), f"ax{axis_index}_scatter_sizes"
                )
                kwargs = {
                ...
                if paths:
                    ...
                    if paths[0].codes is None:
                        ...
                        marker_expression = (
                            f"MplPath({marker_vertices}, {marker_codes})"
                        )
                ...
                for key, value in kwargs.items():
                    if isinstance(value, str) and value.startswith("__ARRAY__:"):
                        ...
                        rendered_kwargs.append(
                            f"{key}={value.removeprefix('__EXPR__:')}"
                        )
                    else:
        ...
        if x_gridlines or y_gridlines:
            ...
            grid_axis = (
                "both" if x_gridlines and y_gridlines else ("x" if x_gridlines else "y")
            )
            grid_kwargs = {
                "axis": grid_axis,
        ...
        for spine_name, spine in ax.spines.items():
            lines.append(f"if {spine_name!r} in {axis_name}.spines:")
    ...
    for legend in fig.legends:
        ...
        bbox = (
            legend.get_bbox_to_anchor().transformed(fig.transFigure.inverted()).bounds
        )
        bbox = tuple(float(value) for value in bbox)
        bbox_anchor = bbox[:2] if np.allclose(bbox[2:], 0.0) else bbox
        fontsize = (
            float(legend.get_texts()[0].get_fontsize()) if legend.get_texts() else None
        )
        kwargs = {
            "loc": _python_value(getattr(legend, "_loc", "best")),
    ...
    payload_source = "\n".join(payload_lines)
    source = "\n".join(lines).replace("__DATA_PAYLOAD_PLACEHOLDER__", payload_source)
...
def plot_flow_pair_closure(
    ...
    if generated_weights is not None:
        ...
        if len(generated_weights) != len(generated):
            raise ValueError("generated_weights must match the generated event array.")
...
def plot_log_prob_cdf_closure(
    ...
    cdf_sigma = np.sqrt(
        flow_cdf * (1.0 - flow_cdf) * (1.0 / len(log_p_mc) + 1.0 / len(log_p_generated))
    )
...
def plot_log_density_truth_binned(
    ...
    if show_scatter:
        if log_p_truth is None or log_p_flow is None:
            raise ValueError(
                "Raw log-density arrays are required for show_scatter=True."
            )
    ...
    ax.step(
        edges, np.r_[flow_step, flow_step[-1]], where="post", lw=2, label="bin mean"
    )
    ax.errorbar(

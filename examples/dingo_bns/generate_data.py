"""Generate a reduced synthetic BNS problem inspired by DINGO-BNS.

This module is deliberately independent of DINGO, Bilby, and LALSuite. It
implements a small, TaylorF2-shaped frequency-domain surrogate observed by
three fixed toy detectors with additive whitened Gaussian noise. The resulting
likelihood is known exactly and is useful for validating dual hNPE--hNDE.

It is not a waveform or detector model for gravitational-wave data analysis.
"""
...
DETECTORS: Final[tuple[str, ...]] = ("H1", "L1", "V1")
FREQUENCIES_HZ: Final[np.ndarray] = np.geomspace(32.0, 1024.0, 6)
OBSERVATION_FEATURES: Final[tuple[str, ...]] = tuple(
    name
    for detector in DETECTORS
    for index in range(len(FREQUENCIES_HZ))
    for name in (
        f"{detector}_f{index:02d}_re",
        f"{detector}_f{index:02d}_im",
    )
)
PRIOR_LOW: Final[np.ndarray] = np.asarray(
    [0.7, 0.0, -np.pi, -0.01, 20.0],
    dtype=np.float64,
)
PRIOR_HIGH: Final[np.ndarray] = np.asarray(
    [1.0, 2000.0, np.pi, 0.01, 80.0],
    dtype=np.float64,
)
FIXED_CHIRP_MASS_MSUN: Final[float] = 1.19786
SOLAR_MASS_SECONDS: Final[float] = 4.925490947e-6
DETECTOR_RESPONSES: Final[np.ndarray] = np.asarray(
    [
        1.0 + 0.0j,
        0.85 * np.exp(0.35j),
        0.45 * np.exp(-0.65j),
...
DETECTOR_DELAYS_S: Final[np.ndarray] = np.asarray(
    [-0.002, 0.002, 0.006],
    dtype=np.float64,
)
...
def waveform_mean(theta: np.ndarray) -> np.ndarray:
    """Return the deterministic reduced three-detector strain mean.

    The source phase contains point-particle terms through 2PN and the leading
    effective tidal correction. Detector responses and time delays are fixed
    context rather than inferred parameters.
    """

    rows = _as_rows(theta, width=len(THETA_FEATURES), name="theta")
    mass_ratio, lambda_tilde, phase, geocent_time, distance = rows.T
    ...
    total_mass = FIXED_CHIRP_MASS_MSUN / (symmetric_mass_ratio ** (3.0 / 5.0))
    frequency = FREQUENCIES_HZ.reshape(1, -1)
    velocity = (np.pi * total_mass.reshape(-1, 1) * SOLAR_MASS_SECONDS * frequency) ** (
        1.0 / 3.0
    )
    eta = symmetric_mass_ratio.reshape(-1, 1)
    point_particle = (
        1.0
        + (3715.0 / 756.0 + 55.0 * eta / 9.0) * velocity**2.0
        - 16.0 * np.pi * velocity**3.0
        + (15293365.0 / 508032.0 + 27145.0 * eta / 504.0 + 3085.0 * eta**2.0 / 72.0)
        * velocity**4.0
    )
    tidal = -39.0 / 2.0 * lambda_tilde.reshape(-1, 1) * velocity**10.0
    source_phase = (
        -phase.reshape(-1, 1)
        - np.pi / 4.0
        + 3.0 / (128.0 * eta * velocity**5.0) * (point_particle + tidal)
        + 2.0 * np.pi * frequency * geocent_time.reshape(-1, 1)
    )
    amplitude = (
        1.4 * (40.0 / distance.reshape(-1, 1)) * (frequency / 100.0) ** (-7.0 / 6.0)
    )
    source = amplitude * np.exp(1j * source_phase)
    delay_phase = np.exp(
        2.0j
        * np.pi
        * DETECTOR_DELAYS_S.reshape(1, -1, 1)
        * FREQUENCIES_HZ.reshape(1, 1, -1)
    )
    network = (
        source[:, np.newaxis, :] * DETECTOR_RESPONSES.reshape(1, -1, 1) * delay_phase
    )
    return np.stack((network.real, network.imag), axis=-1).reshape(len(rows), -1)
...
def generate_datasets(
    ...
    *,
    ...
    manifest = {
        "example": "dingo-inspired-reduced-bns",
        "seed": int(seed),
        ...
        "fixed_chirp_mass_msun": FIXED_CHIRP_MASS_MSUN,
